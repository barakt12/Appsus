export function MailPreview({ props, mail }) {
  ...
  return (
    <article
      className={`preview-container ${isActiveMailClassName}`}
      onClick={() => onSetActiveMail(id)}
    >
      <span
        className={`star ${starClass}`}
        onClick={(ev) => onToggleMarkMail(ev, id)}
      >
        {star}
      </span>
      ...
      <div className='content'>
        <span
          className={`subject ${readingClassName} ${isActiveMailClassName}`}
        >{`${subject} - `}</span>
        <span className={`body ${isActiveMailClassName}`}>{body}</span>
      </div>
      {/* TODO - FORMAT_DATE */}
      <span className='mail-date'>{sentAt}</span>
      <div className='trash-container' onClick={(ev) => onDeleteMail(ev, id)}>
      ...
      >
        <img src={`apps/mail/assets/img/${readingClassName}-mail-icon.png`} />
      </div>
    </article>
  )
}
